...
public class ConnectFailHandle extends MyObservable<ConnectResultHandle> implements ConnectResultHandle, MyObserver<Reachable> {
    ...
    public void handle(){
        ...
        if(currentTimes%2==1){
            ...
            if(currentTimes<App.getMaxTry()){
                ...
                connectManager.notifyObservers(new ErrorHandle("无法连接p.nju.edu.cn"));
            }
        }
    }
    ...
    public void update(MyObservable myObservable, Reachable data) {
        ...
        if(data.isInternetAvailable()){
            ...
            if (data.isCaptital()) {
                System.out.println("需要登录的网络");
                ConnectManager.setStatus(ConnectManager.Status.OFFLINE);
                StatusNotificationManager.showStatus(App.isInPortalWiFiSet(ssid)||App.isInSuspiciousWiFiSSIDSet(ssid) ? "校园网" : "需要登录的网络");
                connectManager.notifyObservers(new UnknownNetHandle(ssid));
            }
            else{
                //尚未完成
     ...
     //   StatusNotificationManager.showStatus(App.isInPortalWiFiSet(ssid)||App.isInSuspiciousWiFiSSIDSet(ssid)? "校园网" : "未知网络");
    }
